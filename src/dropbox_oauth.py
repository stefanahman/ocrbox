    ...
    def do_GET(self):
        ...
        if parsed_path.path == "/oauth/callback":
            ...
            if not auth_code or not state:
                ...
                self.send_header("Content-type", "text/html")
                self.end_headers()
                self.wfile.write(b"<h1>Bad Request</h1><p>Missing authorization code or state</p>")
                return
            ...
            html = f"""
            <html>
            <head><title>OCRBox Authorization</title></head>
            <body>
                <h1>OCRBox - Dropbox Authorization</h1>
    ...
    def exchange_code_for_token(self, auth_code: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if self.allowed_accounts:
                if account_id not in self.allowed_accounts and account_email not in self.allowed_accounts:
                    logger.warning(
                        f"Account not in allowlist: {account_email} ({account_id})"
                    )
                    return False, f"Account {account_email} is not authorized to use this service."
